...
class DebugHook(Hook):
    ...
    def on_case_end(self, res: CaseResult):
        if res.is_skip:
            ...
            print("\n".join([self.padding + i for i in DebugHook.format_case(res, "case")]))

    def on_setup_end(self, res: CaseResult):
        print("\n".join([self.padding + i for i in DebugHook.format_case(res, "setUp")]))

    def on_teardown_end(self, res: CaseResult):
        print("\n".join([self.padding + i for i in DebugHook.format_case(res, "tearDown")]))
    ...
    def format_case(res: CaseResult, case_type: str) -> list[str]:
        ...
        for step in res.before_case_steps:
            lines.extend(["  " + i for i in DebugHook.format_step(step, "beforeCase step")])
        for step in res.pre_steps:
            lines.extend(["  " + i for i in DebugHook.format_step(step, "case preStep")])
        for step in res.steps:
            lines.extend(["  " + i for i in DebugHook.format_step(step, "case step")])
        for step in res.post_steps:
            lines.extend(["  " + i for i in DebugHook.format_step(step, "case postStep")])
        for step in res.after_case_steps:
            lines.extend(["  " + i for i in DebugHook.format_step(step, "afterCase step")])
        ...
        for sub_step in step.sub_steps:
            ...
            for expect_result in sub_step.assertions:
                if expect_result.is_pass:
                    DebugHook.append_val_to_key(sub_step.res, expect_result.node, "<GREEN>{}<END>".format(expect_result.expect))
                else:
                    DebugHook.append_val_to_key(sub_step.res, expect_result.node, "<RED>{}<END>".format(expect_result.expect))
